...
const TYPE_FILTERS = [
  ...
  { value: 'SHARED_ALBUM', label: 'Shared Albums' },
...
export function DashboardPage() {
  ...
  const filtered = {
    accounts: accounts.filter((a) => {
      if (filter !== 'ALL' && filter !== 'SHARED_ALBUM' && a.type !== filter) return false;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content className="z-50 min-w-[180px] rounded-lg border border-border bg-card shadow-lg p-1" sideOffset={4} align="end">
              <DropdownMenu.Item className="flex items-center gap-2 px-3 py-2 text-sm rounded-md cursor-pointer hover:bg-secondary" onSelect={() => setShowAddAccount(true)}>
                ☁️ Storage Account
              </DropdownMenu.Item>
              <DropdownMenu.Item className="flex items-center gap-2 px-3 py-2 text-sm rounded-md cursor-pointer hover:bg-secondary" onSelect={() => setShowAddAlbum(true)}>
                📂 Shared Album
              </DropdownMenu.Item>
